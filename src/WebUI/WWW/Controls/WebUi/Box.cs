...
namespace WebExpress.Tutorial.WebUI.WWW.Controls.WebUi
{
    ...
    public sealed class Box : PageControl
    {
        ...
        {
            Stage.Description = @"The `Box` control is an enclosing frame around content that belongs together, used to set that content apart from the page, to organize it, or to draw the eye to it. A `ControlCard` is a surface with a filled header bar and a footer, a `ControlCallout` is a colored note, a `ControlSection` draws no frame at all - the box sits between them. It draws **exactly one frame**, chosen through `Layout`, around content it otherwise leaves alone. Which frame is the whole statement: a hairline groups, a dashed line marks something provisional, a raised surface lifts content out of the flow, a bar on the leading edge points at it without enclosing it. The body is composed from three sources - fragments registered for `SectionBoxPreferences` and `SectionBoxPrimary`, the controls added directly, and fragments registered for `SectionBoxSecondary` - and the sections resolve against the runtime type of the box, so a plugin that contributes to a box declares `[Scope<TheBoxType>]` and reaches exactly that box. This is how a page assembles itself from parts that do not know each other. The box is also available as an add-on of the WYSIWYG editor, and the reading view of that add-on is this very control.";
