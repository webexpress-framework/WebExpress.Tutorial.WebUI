...
{
    /// <summary>    
    /// Represents the text template of a table control for the tutorial.    
    /// </summary>    
    [Title("Text-Template")]
    [Scope<IScopeGeneral>]
    [Scope<IScopeControl>]
    ...
    public sealed class Text : PageControl
    {
        ...
        public Text()
        {
            ...
            Stage.Description = @"The `Text` template defines how textual values are displayed within a table column. It supports both a ReadOnly mode for presenting static text and an Edit mode that allows users to modify the value directly within the table. This makes the template suitable for simple text fields, inline editing scenarios, and cases where placeholders or text‑specific styling are required.";
        ...
        {
            yield return new ControlTableColumnTemplate("myColumn1", new ControlTableTemplateText()
            {
                Editable = editable,
                Color = color,
