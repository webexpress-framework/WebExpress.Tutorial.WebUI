...
{
    /// <summary>
    /// Represents the color template of the REST table control for the tutorial.
    /// </summary>
    ...
    public sealed class Color : PageControl
    {
        ...
        /// </param>
        public Color(IPageContext pageContext, ISitemapManager sitemapManager)
        {
            Stage.AddEvent(Event.START_INLINE_EDIT_EVENT, Event.SAVE_INLINE_EDIT_EVENT, Event.END_INLINE_EDIT_EVENT);

            Stage.Description = @"The `Color` template displays color values within a REST table column: a swatch in ReadOnly mode and a color picker in Edit mode. The endpoint declares the template on the column, including an optional tooltip.";
            ...
            [
                new ControlDataTable("myTable")
                {
                    ...
                    .DataService<MonkeyIslandCharacterTableColor>(),
            ];
