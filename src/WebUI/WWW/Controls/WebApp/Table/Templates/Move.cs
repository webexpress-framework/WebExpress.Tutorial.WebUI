...
namespace WebExpress.Tutorial.WebUI.WWW.Controls.WebApp.Table.Templates
{
    ...
    public sealed class Move : PageControl
    {
        ...
        /// </param>
        public Move(IPageContext pageContext, ISitemapManager sitemapManager)
        {
            Stage.AddEvent(Event.START_INLINE_EDIT_EVENT, Event.SAVE_INLINE_EDIT_EVENT, Event.END_INLINE_EDIT_EVENT);

            Stage.Description = @"The `Move` template presents a dual-list picker within a REST table column, moving items between an available and a selected side in Edit mode. The selectable items travel embedded in the column template.";
            ...
            [
                new ControlDataTable("myTable")
                {
                    ...
                    .DataService<MonkeyIslandCharacterTableMove>(),
            ];
