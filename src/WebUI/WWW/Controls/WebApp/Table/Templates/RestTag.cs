...
{
    /// <summary>
    /// Represents the rest tag template of the REST table control for the tutorial.
    /// </summary>
    ...
    public sealed class RestTag : PageControl
    {
        ...
        /// </param>
        public RestTag(IPageContext pageContext, ISitemapManager sitemapManager)
        {
            Stage.AddEvent(Event.START_INLINE_EDIT_EVENT, Event.SAVE_INLINE_EDIT_EVENT, Event.END_INLINE_EDIT_EVENT);

            Stage.Description = @"The `RestTag` template presents free-text tags within a REST table column, with autocomplete suggestions served by a REST endpoint in Edit mode. The endpoint declares the template on the column, including the suggestion endpoint.";
            ...
            [
                new ControlDataTable("myTable")
                {
                    ...
                    .DataService<MonkeyIslandCharacterTableRestTag>(),
            ];
