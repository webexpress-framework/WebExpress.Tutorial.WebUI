...
namespace WebExpress.Tutorial.WebUI.WWW.Controls.WebApp.Table.Templates
{
    ...
    public sealed class Editor : PageControl
    {
        ...
        /// </param>
        public Editor(IPageContext pageContext, ISitemapManager sitemapManager)
        {
            Stage.AddEvent(Event.START_INLINE_EDIT_EVENT, Event.SAVE_INLINE_EDIT_EVENT, Event.END_INLINE_EDIT_EVENT);

            Stage.Description = @"The `Editor` template renders rich text within a REST table column and opens the full editor in Edit mode. The endpoint declares the template on the column.";
            ...
            [
                new ControlDataTable("myTable")
                {
                    ...
                    .DataService<MonkeyIslandCharacterTableEditor>(),
            ];
