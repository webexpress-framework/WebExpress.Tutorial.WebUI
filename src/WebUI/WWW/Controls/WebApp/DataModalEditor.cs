...
The control is the dialog rather than a form somebody else opens as one, because a writing surface is only right at that size: the title is an editable field on the dialog's own title bar, the body fills its content with no scrollbar of its own, and the footer bar reads *write | preview · presence · state · ⋯ · publish · close*. It renders closed and is opened by a trigger addressing its id.

Type into the body and watch the footer; then close the dialog without publishing and open it again — the draft is resumed, while the record's published text is unchanged until publish is pressed. The switch at the left end of the bar shows the body the way the readers will get it — the reading view `ControlContent` builds from the same value — without publishing to find out.";
            ...
            (
                "Preview",
                "Offers the reading view of the body on the footer bar: the switch at its left end puts what publishing would show — built from the editor's value by `ControlContent`, with the add-on frames, the column resizers and the guard paragraphs of the writing surface stripped — in the place of the surface, and takes it away again. It follows the text while it is showing, so in a shared document the other authors' typing arrives in it too. It is on by default and independent of the draft; turned off, the bar carries no switch and the dialog holds no reading view.",
                "Preview = _ => false",
                [.. Sample("editor-nopreview", "Without a preview", control => control.Preview = _ => false)]
            );
