...
| `ControlDataModalEditor` | Fullscreen dialog for writing one document — a title on the dialog's own title bar and a rich-text body filling its content. It separates the two things a single save button normally has to pretend are one: every change is written to a draft endpoint within a second of the typing stopping, and the submit publishes through the record endpoint, which applies the text and ends the draft in its own transaction. The footer bar carries a switch to the reading view of the body (what publishing would show, built by `ControlContent`), the save state, who else is in the document, and an overflow menu holding the discard. Without a draft endpoint — or with `Draft` turned off — it is an ordinary edit dialog with save and cancel. |
